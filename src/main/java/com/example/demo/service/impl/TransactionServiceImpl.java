...
public class TransactionServiceImpl implements TransactionService {
   ...
   private final TransactionRepository transactionRepository;
    ...
    public TransactionResponse performTransaction(TransactionRequest trnRequest) throws RecordNotFoundException {
        ...
        Client reciever = clientService.findByEmail(trnRequest.getDestinationEmail());
        ...
        if(TrnUtil.compareAmounts(sender.getBalance(),trnRequest.getAmount())){
            ...
            reciever.setBalance(BigDecimal.valueOf(reciever.getBalance().doubleValue()+trnRequest.getAmount()));
        }
        Currency trnCurrency = currencyService.findByName(trnRequest.getCurrency().getCurrencyName());
        Transaction trn = convertor.toTransaction(sender,reciever,trnCurrency,new BigDecimal(trnRequest.getAmount()),trnRequest.getReason());
        transactionRepository.save(trn);
        return new TransactionResponse(String.format("Transaction to %s was executed successfully", reciever.getEmail()));
